...
export default function PokemonDetail({ params }) {
    ...
    useEffect(() => {
      const fetchPokemonDetails = async () => {
        try {
          const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${JSON.parse(params.value).id}`);
            ...
            <div className="p-8">
              <div className="flex flex-col md:flex-row gap-8 items-center">
                ...
                <div className="w-full md:w-1/2">
                  ...
                  <p className="text-xl text-gray-500 mb-4">#{String(pokemon.id).padStart(3, "0")}</p>
                      ...
                      {pokemon.stats.map(({ base_stat, stat }) => (
                        <div key={stat.name}>
                          <div className="flex justify-between mb-1">
                            ...
                            <span className="text-sm text-gray-500">{base_stat}</span>
                          </div>
